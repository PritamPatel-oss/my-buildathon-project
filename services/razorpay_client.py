...
def create_payment_link(amount_inr: float, email: str, reference_id: str) -> dict:
    """
    Creates a Razorpay test-mode Payment Link and returns {success, response}.
    Amount is converted from INR to paise (Razorpay's base unit).
    """
    try:
        client = _get_client()
        payload = {
